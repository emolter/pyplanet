...
C Uses Orton's H2-He-CH4 tables for hydrogen opacity
C Added CO12 and CO13; but CO13 is still not right, it looks like
C  **NOTE lres, and  lco,freqco1,freqco2 have to be set in nammon_co.dat 
C
C f77 -O -o sus susnep_v2012_orton_CO.for
C  sus    to run it
C
C   ifort -132 susnep_v2012_orton_CO.for -o sus.exe 
C ./sus.exe       to run it
...
C 
C  AS 2,VOL2:NPAR.DAT       PARTIAL PRESS DATA ARE CALCUL AND PUT IN THIS 
C  AS 3,VOL2:NTPZ.DAT        TEMP PRESS ALT ARE CALC AND PUT HERE 
C  AS 5,VOL2:NAMMON.DAT    INPUT 
C  AS 6,SPR: 
C  AS 7,VOL2:KAKAR.DAT    CONTAINS AMMONIA LINES AND CP FOR H2 
C  M5 EQ 0  CALCULATE TABLES; NE 0 ASSUMES TABLES EXIST ALLREADY; is nowadays always set to zero
	...
	real*4  F0(311),S0(311),EL(311),aph3,ah2s,aco,wgths(311)
...
C     VSET ARE WAVENUMBERS (CM-1). 
      DATA CSZA/0.9978,.9911,.9798,.9638,.9428,.9165, 
...
C  the nammon input files are for CO12, CO13, or HCN
C NWN = 35
C the vsets are calculated in program
      OPEN(5,NAME='nammon_co.dat')
      OPEN(6,NAME='test1.dat')
      Open(7,NAME='kakar.dat')
      open(11,NAME='orton_H2_long.tables',form='formatted')
...
C Den and Alt are no longer in use; I left DEN here because it is passed through in common blocks 
       CZB=0.0
      DEN=7.2e6
      NOPR=0 
      NOPR2=0 
...
C 
      READ(5,779) MII
      READ(5,778) lco,freqco1,freqco2
C lco = 0, 1, 2 for continuum, J=1-0, 2-1 lines; read in in GHz; convert to Hz
      freqco1=freqco1*1.0E9
      freqco2=freqco2*1.0E9
      if(lco.eq.1) fr0=freqco1
      if(lco.eq.2) fr0=freqco2
C       write(6,781) lco,fr0
 781   format(I5,E20.8)
  779 FORMAT(I2) 
      READ(5,773) SIZE
C      WRITE(6,772) SIZE
...
C  INTERMEDIUM: IQ=1; REB=1.0.; FP=S1
C  IQ=2: TEMP FOR EQUIL. CASE
      READ (5,778) IQ,REB,FP 
C      WRITE(6,778) IQ,REB,FP 
  778 FORMAT(I2,2F12.7) 
C 
      DO 1357 I=1,18 
      RR(I)=ACOS(CSZA(I)) 
      RR(I)=SIN(RR(I)) 
C      WRITE(6,1345) CSZA(I),DCSZA(I),RR(I) 
 1345 FORMAT(3E15.5) 
 1357 CONTINUE 
      DO 2345 J=1,16 
      DO 2345 K=1,J 
...
C 
      READ(5,1041) M5,NL
 1041 FORMAT(I1,1X,I5) 
C      WRITE(6,1046) M5,NL 
...
C choose one of CO12, CO13, HCN 
C  we use vset here for freq, and calculate vnu
      vset(1)=fr0+15000.*1.0E6
      do n=2,nwn
         vset(n)=vset(1)-(n-1)*1.0E6
      enddo   
 1003  continue
      DO 200 N=1,NWN 
      freq=VSET(N) 
...
C        if(ph2o.ge.1.0E-15) CALL H2O(VNU,T,AH2O)
C        AH2O=0.0
         if(ph2s.ge.1.0E-15) CALL absbr_H2S2(t,tpr,vnu,AH2S)
C        Ah2S=0.0
         if(pph3.ge.1.0E-15) CALL absbr_ph3(t,tpr,vnu,APH3)
C        aph3=0.0
 911     continue
C
...
C     INTENSITY. 
      ...
      CS=CSZA(JJ) 
      DC=DCSZA(JJ)
C      CALL BRIGHT(VNU,FREQ,NL,CS,BRI) 
      CALL BRIGHTor(VNU,FREQ,NL,CS,BRI) 
      STR(JJ)=BRI	
...
C 
      ...
      XNADIR(N)=STR(1)
  200 CONTINUE
C  write: frequ, Tb at center disk, disk-averaged, disk-averaged and binned over 3 frequ.
      DO 290 N=2, NWN-1
      FREQ=C*VSET(N)
      x=(xtemp(n-1)+xtemp(n)+xtemp(n+1))/3.
      WRITE(6,5768) freq,XNADIR(N), XTEMP(N),x
  290 CONTINUE
 5768   FORMAT(4E20.8)
      STOP 
      END
 ...
 888  FORMAT(2X,'FREQ,CMU,BR,T ARE ',/,4E15.5) 
     ...
     +co(7000),co13(7000),hcn(7000)
      ...
      DIMENSION TEMPHE(7000),TEMPXG(7000),HELIUM(7000),XH(7000)
        ...
        dimension tmpph3(7000),tmpco(7000),tmphcn(7000)
...
C       write(6,19) JMAX
     ...
     +  AMMON(II),WATER(II),H2S(II),SOLN(II)
        if(CH4(II).le.1.0E-15) CH4(II)=0.0
        if(AMMON(II).le.1.0E-15) AMMON(II)=0.0
        if(WATER(II).le.1.0E-15) WATER(II)=0.0
        if(H2S(II).le.1.0E-15) H2S(II)=0.0
        if(SOLN(II).le.1.0E-15) SOLN(II)=0.0
...
C ***NOTE: check on pco in last loop; do 50 M=1,----
...
C 
      ...
      ZZZ=ZZZ+DELZ*1.0E-5 
      TPR=PR
      WRITE(2,7) TPR,PH2,PHE,PNH3,PH2O,PCH4,PH2S,PPH3,PCO,PCO13,PHCN
      IF(TT.GT.TFLAT) 
...
C
C
       SUBROUTINE ABSX_H2S(AON,T,P,VNU,ZH2S)
CC This is the old routine
...
C*******************************************************************
	...
	REAL*4 C,ZH2S,DV,VNU
        ...
        DATA Q0 /2.711, 2.5237, 2.2605, 1.8164, 1.3778/ !LOG PF AT 300,225,150,75, 37.5K  
        DATA F0(1),S0(1),EL(1)
     &  /168762.7624, -2.8373, 13.7463/
        DATA F0(2),S0(2),EL(2)
     &  /216710.4365, -3.0225, 51.1402/
        DATA F0(3),S0(3),EL(3)  
     &  /300505.560, -2.3413, 107.3683/
        DATA F0(4),S0(4),EL(4)  
     &  /369101.450, -2.1283, 95.0564/
        DATA F0(5),S0(5),EL(5)
     &  /369126.912, -2.5921, 170.3358/
        DATA F0(6),S0(6),EL(6)
     &  /393450.490, -2.7368, 38.0161/
        DATA F0(7),S0(7),EL(7)
     &  /407676.730, -2.1415, 263.7392/
        DATA F0(8),S0(8),EL(8)
     &  /424314.820, -2.6990, 182.6486/
        DATA F0(9),S0(9),EL(9)
     &  /452390.330, -2.6152, 0.000/
        DATA F0(10),S0(10),EL(10)
     &  /493362.160, -2.6702, 375.4318/
        DATA F0(11),S0(11),EL(11)
     &  /505565.230, -2.2024, 38.2978/
	P=P*1000.0		!MILLIBAR
	C=2.99791E1
	F=VNU*C*1.0E3                  !FREQU IN MHZ
	AON=0.0
     ...
     & /(1.-EXP(-1.5998E-07*F0(N)))
       AON=AON + TCOR*SHAPE2(P,T,F,F0(N))*(10.**S0(N))
       ENDDO
       AON=AON*10**(Q0(1)-QLOG2(Q0,T)) !T DEPENDENCE OF PARTITION FUNCTION
       AON=0.72435E5*AON*P*ZH2S/T       !YIELD ABSORPTION COEFFICIENT IN 1/cm
...
10     CONTINUE
      ...
      SUBROUTINE SAJh2s(N,T3,GM)                               
	real*4 t3,gm,vline,fjk,ph2,phe,ph2s,pnh3,pch4,ph2o,pph3,PCO,PCO13,PHCN
      COMMON/U1/PH2,PHE,PNH3,PH2O,PCH4,PH2S,PPH3,PCO,PCO13,PHCN 
...
C                                                                               
C                                                                               
      SUBROUTINE SFJH2S(N,T3,V,VLINE,GM,FJK)                                
	real*4 t3,gm,vline,fjk,ph2,phe,ph2s,pnh3,pch4,ph2o,pph3,PCO,PCO13,PHCN
      COMMON/U1/PH2,PHE,PNH3,PH2O,PCH4,PH2S,PPH3,PCO,PCO13,PHCN
C                                                                               
C     FJK IS THE BEN REUVEN LINE SHAPE, FROM DEBOER AND STEFFES
C                                                                               
C      X=PH2+PHE+PH2O+PNH3+PCH4                                                  
C  IF X.LT.1 ATM VAN VLECK-WEISKOPF LINE SHAPE CAN BE                           
C  USED; HOWEVER, BEN REUVEN IS STILL BETTER OUT IN THE                         
C  WINGS OF THE AMMONIA LINES. 
c     A2 IS THE COUPLING FACTOR ZETA(J,K) IN GHZ.                               
      A2=(1.96*PH2*T3)+(3.26*T3*PH2S)+(0.355*T3*PHE)
C     A4 IS THE PRESSURE SHIFT DEL (GHZ)                                        
	A4=1.28*PH2S
C     SQUARES OF VARIOUS TERMS.                                                 
	A3=VLINE*VLINE
...
C                                                                               
	...
	z=y*y  + 4.0*v*v*GM*GM
      ...
      END                                                                       
C
       SUBROUTINE ABSBR_h2s2(T,P,VNU,AH2S)
C
C THIS SUBROUTINE RETURNS QUANTITY AON, WHICH WHEN MULTIPLIED
C BY THE VOLUME MIXING RATIO, GIVES THE ABSORPTION COEFFICIENT
...
C          xshape=vvwshape
          xshape=shape
C          write(6,*) j,T,fr0,wd,voigt(x,a)
        call absx_coline(j,aon,t,pin,f,xshape)
C         write(6,*) j,aon,t,f,xshape
...
C       DFC=188.57*P/(T**0.75)
	...
	SHAPE=SHAPE/3.1415926536		!DIVIDE BY PI
       RETURN
       END
       SUBROUTINE ABSBR_PH3(T,P,VNU,APH3)
C
C THIS SUBROUTINE RETURNS QUANTITY AON, WHICH WHEN MULTIPLIED
C BY THE VOLUME MIXING RATIO, GIVES THE ABSORPTION COEFFICIENT
