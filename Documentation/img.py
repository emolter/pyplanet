...
class Img:
    ...
    def NepLat(self,tip='auto',reference='ref'):
        """ad hoc function to draw specific latitude lines for Neptune VLA observations"""
    ...
    def editImageLat(self,latmod,Tmod,limb=[0.0,0.0]):
        """This only works for negative latitudes to pole.  Need to make northern,southern,straddle + bands.
            ...
            Tmod is the value at the pole (K)
            limb is the [b-pivot and slope (K/|b|)] to provide a linear slope in b"""
        import planet
        import atmosphere
        atm = atmosphere.atmosphere('neptune','/Users/ddeboer/Documents/Projects/Planets/pyPlanet')
        atm.run()
        p = planet.planet('functions')
        bdata = p.bRequest(self.header['b'][0],block=[1,1])
        if len(bdata) != np.size(self.data):
        ...
        for i in range(len(bdata)):
            ...
            if pclat < lat_pcmod:
                ...
                nedit+=1
        for px in pixel_modlist:
            bval = bdata[px[0]][px[1]]
            Tnew = Tspot + limb[1]*(limb[0] - bval)
