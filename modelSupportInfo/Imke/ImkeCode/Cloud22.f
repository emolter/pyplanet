...
C**  rate can vary from the pure dry (lapse=0.0), to dry plus some    **
C**   percent of   
C**  the wet correction, to the appropiate wet adiabat (lapse=100). 
C**   Condensation  
...
C**  from PART in Imke's RT program THIS SEEMS TO WORK  
C**
C**  Cloud18 contains a variation in g with altitude (depth) --
C** g propto 1/R^2. Deepest layers in Neptune 6% different. 
C** easiest to run Cloud17 first; find the approximate altitude of the 
C** starting depth, calculate G at the starting depth and use that as 
C** input parameter for g
C** Note that ENRHE = He/H2 ratio; not enrichment factor.                                                   
C
C** Cloud 19:  here we try to expand the TP above the tropopause, 
C   in particular for Neptune to calculate CO lines.
C  Since TP is not adiabatic anymore at altitudes above about 0.5-1 bar, 
C  I fudge the cut-off at a different tropospheric temp. 
C  For Neptune I choose 80 K, about 1.5 bar. 
C I then read in a TP profile that is fixed; and calculate the corresponding
C Z's and fix the abundances, etc.
C
C  You need to twiddle the lower T, P until a good match to upper profile
C  is reached near the 80 K, 1.5 bar level.
C  Then edit the IMKEIN file (cut out the overlapping PT range; but note CH4 abundance
C  from 80 K down to where you get 78 K again), 
C and run the imke.f program (after inserting top line with nr of lines in IMKEIN.DAT) to get
C  the right input file for the RT programs
C An adapt the CLDDEN file
C
C  cloud20 is the same as 19, but I deleted extraneous stuff (like clouds in stratosphere)
C cloud22 is the same as cloud20, but the variation in G is deleted.
C  also extended in size from 1200 to 2500
C 
C***********************************************************************
C
C f77 -O -o cloud Cloud20.f
C ifort -132 Cloud22.f -o cloud.exe
...
C
      DIMENSION C(2500)
      DIMENSION P(2500), T(2500)
      DIMENSION XH2O(2500),XNH3(2500),XH2S(2500),XCH4(2500),XAR(2500)
...
C      OPEN (UNIT=9, NAME='lindal_nep.dat', STATUS='OLD')
C      OPEN (UNIT=9, NAME='fletcher_best.dat', STATUS='OLD')  ;for Neptune
       open(UNIT=9,NAME='lindal_jup_Gal_inv.dat',STATUS='OLD')
...
C
      ...
      RR = R
C
C  Read in the interpolation data for the subroutuines STPA, STPWLT,
...
C
      READ (2,20) PLANET,radius,depth
   20 FORMAT (2A4,2F10.2)
C  radius and depth of deepest layer are used to calculate G; but these parameters are no longer used in this version
...
C
C Lapse:  'The lapse rate is the dry adiabatic lapse rate + lapse% of the wet corrrection.
C super: lapse rate is multiplied by super
...
C  the planets HE/H2 value (=ENRHE), and assumed enrichments relative to their
...
C
C      G=G0*(radius*radius)/((radius-(depth-Z/1.0E5))*
C     +(radius-(depth-Z/1.0E5)))
      A1 = G * W
      A2 = -A1 / CP
      A3 = -A1 / R
      A4 = XH2O(1) / ( 1.0D0 - C(1) )
      A5 = CP * R
C      write(3,888) G,G0,W,A1,A2,A3,Z,radius,depth
 888  format(9E12.5)
C
...
C
      WRITE(3,100) PLANET, ICASE, Radius, Depth
  100 FORMAT( '1', 4X, 'Intial conditions for ', 2A4, '  Case ',
     1  I3, '  CLOUD22 Model, (radius, depth) ',2F10.3/' '/ )
...
C
  ...
  250 FORMAT('1', 3X, 2A4, 3X, 'CASE ', I3, 3X, 'CLOUD18 Model ',
     1  ' '/ ' ')
      WRITE(3,255)
  255 FORMAT(5X, 'Thermal Structure/Solution Cloud'/ ' ')
c      WRITE(3,260)
c  260 FORMAT(69X, 'NH3 - H2O Solution'/ ' '/ 3X, 'Z', 7X, 'T', 8X, 'P',
c     1  6X, 'DT/DZ', 5X, 'Cp H2', 6X,'fp', 5x, 'Cp Atm', 5X, 'W', 8X, 'Conc',
c     2  5X, 'Delta', 3X, 'Conc H2S'/ 2X, '(km)', 4X, '(K)', 5X,
c     3  '(bars)', 3X, '(K/km)', 2X, '(Erg/MolK)', 2X, 7X, '(Erg/MolK)', 2X,
c     4  '(g/M)', 6X, ' NH3', 5X, 'Conc', 3X, '(Mol/Lit)'/ ' ')
...
C
 ...
 500  FORMAT(1X,F5.1,2X,F7.3,2X,1PE10.4,2X,0PF6.3,2X,1PE9.3,2x, 
  ...
  510 FORMAT(1X,F5.1,2X,F6.2,2X,1PE9.3,6(2X, 1PE8.2, 2X, 1PE9.2))
     ...
     1  DCH4S, DARS
  520 FORMAT( 1X, F6.1, 2X, F9.2, 2X, 1PE9.3, 7(2X, 1PE9.3) )
      WRITE(8,530) ZZ, T(J),PP,XH2,XHE,XCH4(J),XNH3(J),XH2O(J),
     1  XH2S(J), DSOLN, DNH4SH
  530 FORMAT(1X, F6.1, 2X, F9.2, 2X, 1PE11.4, 2(2X, 0PF5.3),
     1  6(2X, 1PE8.2) )
C
C***********************************************************************
C
...
C
  540 GO TO 290
C
  550 WRITE(3,560) J,P(J-1),T(J)
      WRITE(4,560)
      WRITE(7,560)
  560 FORMAT( ' '/ 5X, 'Next Temperature <= Tropopause Temperature'/
     1  5X, 'Execution Terminated',I5,2E12.5)
  570 CONTINUE
C
C  Set CH4 to observed value of 3.5E-4; for neptune
C      XCH4(J-1)=3.50E-4
C      XCH4(J)=3.50E-4
      k=j-1
      J=K
...
C
      ...
      XS = XH2S(J)
      XA = XAR(J)
C      write(3,99) j,P(j),P(j-1),DP,DT,W,RH,DZ,Z
C 99   format(I5,8E12.5)
C
C  Compute new partial pressures
C
      PPH2O = XH2O(J) * P(J)
...
C  **************
      ...
      PP = P(J) * Q3
C
      WRITE(3,500) ZZ, T(J),PP,DTDZZ,CPH2,fp,CP,W,DZZ,W,W
      WRITE(4,510) ZZ,T(J),PP,XH2O(J),DXH2O,XNH3(J),DXNH3,
     1  XH2S(J), DXH2S, XCH4(J), DXCH4, XAR(J), DXAR
c      WRITE(7,520) ZZ, T(J), PP, DSOLN, DH2OS, DNH4SH, DNH3S, DH2SS,
c     1  DCH4S, DARS
      WRITE(7,520) ZZ, T(J), PP, DSOLN, Dsoln, Dsoln, Dsoln, Dsoln,
     1  Dsoln, Dsoln
      WRITE(8,530) ZZ,T(J),PP,XH2,XHE,XCH4(J),XNH3(J),XH2O(J),
     1  XH2S(J), DSOLN, DNH4SH
 580  continue
...
C      write(7,777) dry,wet,lapse,wetcor,dtdz
C 777  format(5E12.4)
      END IF
C
      RETURN
