...
class planetConfig:
    def __init__(self, planet, configFile, log=None, verbosity=False, printHelp=False):
        ...
        self.toks = {'gasfile': ['gasFile', str], 'cloudfile': ['cloudFile', str], 'gasfilehdr': ['gasFileHdr', int],
                     ...
                     'regridtype': ['regridType', str], 'pmin': ['pmin', float], 'pmax': ['pmax', float], 'omega': ['omega_m', float],
                     'jn': ['Jn', str], 'gm': ['GM_ref', float], 'req': ['Req', float], 'rpol': ['Rpol', float], 'distance': ['distance', float],
                     'rj': ['RJ', float], 'p_ref': ['p_ref', float], 'zonal': ['zonal', str], 'gtype': ['gtype', str],
                     'orientation': ['orientation', str], 'h2state': ['h2state', str], 'doppler': ['Doppler', str], 'h2newset': ['h2newset', str],
                     'water': ['water_p', float], 'ice': ['ice_p', float], 'nh4sh': ['nh4sh_p', float], 'nh3ice': ['nh3ice_p', float],
                     'h2sice': ['h2sice_p', float], 'ch4': ['ch4_p', float], 'limb': ['limb', str]}
    ...
    def setConfig(self, configFile):
        ...
        for line in fp:
            ...
            if tok not in self.toks:
                print('token %s not found'.format(tok))
                continue
            nTokMod += 1
    ...
    def show(self):
        ...
        keys.sort()
        for key in keys:
            s += '\t{:15s}:  {:15s} \t{}\n'.format(key, str(type(self.__dict__[self.toks[key][0]])), str(self.__dict__[self.toks[key][0]]))
        return s
